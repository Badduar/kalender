...
//  Serientermine
// ============================================================
//  Gespeichert wird nur der Serien-Kopf plus eine Regel im RRULE-Format
//  (Teilmenge: FREQ, INTERVAL, BYDAY, BYMONTHDAY, COUNT, UNTIL).
//  Die einzelnen Vorkommen entstehen hier im Browser.
//
//  Gerechnet wird auf Datums-Schluesseln und Berliner Wanduhrzeit.
//  Deshalb bleibt ein woechentlicher 9-Uhr-Termin auch nach der
//  Zeitumstellung ein 9-Uhr-Termin.
// ============================================================
...
export function regelSchreiben(regel) {
  if (!regel || regel.haeufigkeit === "KEINE") return null;
  const stuecke = [`FREQ=${regel.haeufigkeit}`];
  if (regel.intervall > 1) stuecke.push(`INTERVAL=${regel.intervall}`);
  if (regel.haeufigkeit === "WEEKLY" && regel.wochentage?.length) {
    stuecke.push(`BYDAY=${regel.wochentage.map((i) => BYDAY_CODES[i]).join(",")}`);
  }
  if (regel.haeufigkeit === "MONTHLY" && regel.monatstag) {
    stuecke.push(`BYMONTHDAY=${regel.monatstag}`);
  }
  if (regel.anzahl) stuecke.push(`COUNT=${regel.anzahl}`);
  else if (regel.bis) stuecke.push(`UNTIL=${regel.bis.replaceAll("-", "")}`);
  return stuecke.join(";");
}

// Klartext fuer die Anzeige, z.B. "Alle 2 Wochen, Mo und Do"
export function regelText(text) {
  const regel = regelLesen(text);
  if (!regel) return "";

  const einheit = {
    DAILY: regel.intervall === 1 ? "Täglich" : `Alle ${regel.intervall} Tage`,
    WEEKLY: regel.intervall === 1 ? "Wöchentlich" : `Alle ${regel.intervall} Wochen`,
    MONTHLY: regel.intervall === 1 ? "Monatlich" : `Alle ${regel.intervall} Monate`,
    YEARLY: regel.intervall === 1 ? "Jährlich" : `Alle ${regel.intervall} Jahre`,
  }[regel.haeufigkeit] ?? "Wiederholt";

  const stuecke = [einheit];
  if (regel.haeufigkeit === "WEEKLY" && regel.wochentage.length) {
    const namen = regel.wochentage.map((i) => WOCHENTAGE_LANG[i]);
    stuecke.push(namen.length === 1
      ? `am ${namen[0]}`
      : `am ${namen.slice(0, -1).join(", ")} und ${namen.at(-1)}`);
  }
  if (regel.haeufigkeit === "MONTHLY" && regel.monatstag) {
    stuecke.push(`am ${regel.monatstag}.`);
  }
  if (regel.anzahl) stuecke.push(`· ${regel.anzahl} Mal`);
  else if (regel.bis) {
    const { jahr, monat, tag } = schluesselTeile(regel.bis);
    stuecke.push(`· bis ${tag}.${monat}.${jahr}`);
  }
  return stuecke.join(" ");
}
...
// Alle Starttage der Serie zwischen zwei Schluesseln (beide einschliesslich).
// Gezaehlt wird immer ab dem Serienanfang, damit COUNT stimmt.
...
// Letzter Tag der Serie, soweit er feststeht (fuer die Spalte serie_ende).
export function serienEnde(startSchluessel, regelText) {
  const regel = regelLesen(regelText);
  if (!regel) return null;
  if (regel.bis) return regel.bis;
  if (!regel.anzahl) return null;

  // Grosszuegig weit suchen und den letzten Treffer nehmen.
  const tage = serienTage(startSchluessel, regelText, startSchluessel, "2999-12-31");
